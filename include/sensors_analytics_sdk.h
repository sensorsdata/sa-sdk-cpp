...
class Sdk {
 public:
...
#if VS2015
  static bool Init(const wstring& data_file_path, const string& server_url,
      const string& distinct_id, bool is_login_id,
      int max_staging_record_count);
  ...
  static void Bind(const string& key, const string& value);
  static void Login(const string& key, const string& loginId);
  static void Reset(const string& distinct_id, const map<string, string>& identities);
  static void Reset(const map<string, string>& identities);
 ...
 private:
  Sdk(const string &server_url, const string &data_file_path,
      ...
      bool is_login_id, const map<string, string>& identities);
#if VS2015
  Sdk(const string& server_url, const wstring& data_file_path,
      int max_staging_record_count, const string& distinct_id,
      bool is_login_id, const map<string, string>& identities);
#endif // VS2015
...
#if VS2015
  void SetString(const string& property_name, const wstring& value);
#endif // VS2015
